...
The Public lands section includes a master checkbox for all public-land layers available in the current area. The bottom of the drawer lists every active layer in descending session-transfer order, shows browser-reported transferred bytes and request counts, and provides a direct checkbox for disabling each layer. Transfer totals reset on page reload; cached responses and third-party services without Resource Timing size access can report an unavailable size.
